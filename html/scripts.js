...
var noImageUrl = './no_image.png'; // 대체 이미지 경로
...
function initMap() {
    var mapOptions = {
        center: { lat: 37.5665, lng: 126.9780 }, // 서울 시청 중심 좌표
        zoom: 15, // 초기 줌 레벨 설정
...
function setRadius() {
    ...
    if (radius === "none") {
        ...
        map.setZoom(20); // 반경 변경 시 줌 레벨을 20으로 설정하여 약 100m 축척
...
function moveToLocation(location) {
    if (locations[location]) {
        var newCenter = { lat: locations[location].lat, lng: locations[location].lng };
        map.setCenter(newCenter);
        circle.setCenter(newCenter);
        map.setZoom(20); // 이동 후 줌 레벨을 20으로 설정하여 약 100m 축척
        document.getElementById('selected-location').textContent = '현재 선택된 지역 : ' + location;
        fetchAllRestaurantMarkers(newCenter.lat, newCenter.lng, document.getElementById('radius-select').value);
    }
}
...
function searchLocation() {
    ...
    geocoder.geocode({ 'address': address }, function(results, status) {
        if (status === google.maps.GeocoderStatus.OK) {
            ...
            map.setZoom(20); // 검색 후 줌 레벨을 20으로 설정하여 약 100m 축척
            document.getElementById('selected-location').textContent = '현재 선택된 지역 : ' + results[0].formatted_address;
            fetchAllRestaurantMarkers(newCenter.lat(), newCenter.lng(), document.getElementById('radius-select').value);
        } else {
...
window.onclick = function (event) {
    if (!event.target.matches('.dropdown-label') && !event.target.matches('.menu-item')) {
        ...
        if (status === google.maps.places.PlacesServiceStatus.OK) {
            ...
            results.forEach(function (place) {
                var detailsRequest = {
                    ...
                    fields: ['name', 'vicinity', 'formatted_phone_number', 'rating', 'user_ratings_total', 'geometry', 'photos']
                };
                ...
                service.getDetails(detailsRequest, function (placeDetails, status) {
                    if (status === google.maps.places.PlacesServiceStatus.OK) {
                        ...
                        if (placeDetails.rating && placeDetails.user_ratings_total && placeDetails.photos) {
...
function renderRestaurantInfo(restaurants) {
    ...
    restaurants.forEach(placeDetails => {
        ...
        img.src = placeDetails.photos ? placeDetails.photos[0].getUrl({ maxWidth: 80, maxHeight: 80 }) : noImageUrl;
        ...
        phone.textContent = `연락처: ${placeDetails.formatted_phone_number ? placeDetails.formatted_phone_number : 'N/A'}`;
